...
export default function StaffOrderHistoryScreen({ navigation }) {
  ...
  const loadFirst = useCallback(async () => {
    try {
      const raw = await AsyncStorage.getItem(STAFF_TASKS_HISTORY_CACHE_KEY);
      if (raw) {
    ...
    try {
      ...
      console.warn('Staff staff-tasks history failed', e);
      setAllTaskItems([]);
      setTotalPages(1);
    } finally {
      setLoading(false);
  ...
  const onRefresh = useCallback(async () => {
    ...
    try {
      ...
      console.warn('Staff staff-tasks history refresh failed', e);
    } finally {
      setRefreshing(false);
    }
  }, [fetchPage]);
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      console.warn('Staff staff-tasks history loadMore failed', e);
    } finally {
  ...
  const onPressOrder = useCallback(
    (item) => {
      navigation.navigate('StaffOrderDetailHistory', {
    ...
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>
          Lịch sử đơn hàng được phân công cho bạn.
        </Text>
      </View>
